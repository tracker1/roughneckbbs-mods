		...
		if (c != "") {
			switch (c.toString()) {
				...
				case "9":
					sRet += "" + c;
					console.print(c);
					if ((sRet.length >= iMax.length)||((sRet.length == iMax.length-1)&&(sRet.substr(0,1) > iMax.substr(0,1))))
						bDone = true;			
					break;
				case "\r":
				case "\n":
...
function readPrompt() {
	...
	arrPrompts[0] = "\1h\r\1cReading Messages \1h\1k³³ \1n<\1" + "6\1h\1wprev\1n>\1h\1k [\1nnext\1n\1h\1k] [\1nreply\1n\1h\1k] [\1npost\1n\1h\1k] [\1nquit\1n\1h\1k] ³³ (\1y?\1k-\1n\1chelp\1h\1k) \1n\1c: \1n";
    ...
    user.flags4 |= UFLAG_R;	//set redraw flag
	user.flags4 |= UFLAG_P; //set paused/prompt flag
	...
	while (bbs.online && (sRet == "")) {
		...
		if (c == "") 
			...
			switch (c) {
				...
				case "8":
				case "9":
					sRet = inputNumber(c,bbs.smb_last_msg);
					break;
				case "\r":
				case "\n":
	...
	user.flags4 &= ~UFLAG_R; //reset redraw flag
	user.flags4 &= ~UFLAG_P; //reset paused/prompt flag
